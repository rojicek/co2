...
float read_co2()
{
 ...
 float mv_voltage = sensorValue*(3200/1023.0);
 float instConc = -1; //default

 if(mv_voltage > 400.) 
    {
      int mv_voltage_diference = mv_voltage - 400;
      instConc = mv_voltage_diference * 50.0 / 16.0;     
    } 
...
unsigned int htdu21d_readTemp()
{
  ...
  delay(100); //44-50 ms max - we could also poll the sensor
  ...
  unsigned int temperature = ((unsigned int)msb << 8) | ((unsigned int)lsb);
